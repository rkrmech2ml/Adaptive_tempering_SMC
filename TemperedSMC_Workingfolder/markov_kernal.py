...
def markov_kernel(particles,particles_initial, resampled_particles, experiment, n, h, beta):
    ...
    spread1 = np.max(particles_initial[:, 0]) - np.min(particles_initial[:, 0])
    #print(f"Spread in first dimension: {spread1}")
    spread2 = np.max(particles_initial[:, 1]) - np.min(particles_initial[:, 1])
    ...
    print("Max particle0:", np.max(resampled_particles[:,0]))
    print("Min particle0:", np.min(resampled_particles[:,0]))
